...
import { PlayPause } from 'shared/ui';
...
const TopChartCard = ({ song, i, isPlaying, activeSong, handlePauseClick, handlePlayClick }) => (
  <div className={`w-full flex flex-row items-center hover:bg-[#4c426e] ${activeSong?.title === song?.title ? 'bg-[#4c426e]' : 'bg-transparent'} py-2 p-4 rounded-lg cursor-pointer mb-2`}>
    ...
    <div className="flex-1 flex flex-row justify-between items-center">
      <img className="w-20 h-20 rounded-lg" src={song?.images?.coverart || noimg} alt={song?.title} />
          ...
          {topPlays?.slice(0, 5)?.map((artist) => (
            ...
            >
              <Link to={`/artists/${artist?.artist_id}`}>
                <img src={artist?.images?.background || noimg} alt="Name" className="rounded-full w-full object-cover" />
              </Link>
            </SwiperSlide>
          ))}
        </Swiper>
